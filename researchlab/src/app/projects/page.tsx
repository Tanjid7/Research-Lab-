      ...
      <section className="px-6 md:px-12 lg:px-20 mb-8">
        <div className="container-max space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {/* Area */}
            <button onClick={() => setAreaFilter('all')} className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${areaFilter === 'all' ? 'bg-lab-500 border-lab-500 text-white' : 'border-white/10 text-gray-400 hover:border-lab-400/30'}`}>All Areas</button>
            {LAB_CONFIG.researchAreas.map(a => (
              <button key={a.id} onClick={() => setAreaFilter(a.id)} className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${areaFilter === a.id ? 'bg-lab-500 border-lab-500 text-white' : 'border-white/10 text-gray-400 hover:border-lab-400/30'}`}>
                {a.label}
              </button>
            ))}
            ...
            {['all', 'active', 'completed', 'archived'].map(s => (
              <button key={s} onClick={() => setStatus(s)} className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all capitalize ${status === s ? 'bg-lab-500 border-lab-500 text-white' : 'border-white/10 text-gray-400 hover:border-lab-400/30'}`}>
                {s === 'all' ? 'All Status' : s}
              </button>
            ))}
          </div>
